...
#'
#' @export
fct_checkinput <- function(.path, .pb_session = NULL, .pb_id = NULL) {
  ...
  checklist <- list(
    ...
    cols = list(
      setup  = c("trunc_pdf", "n_iter", "ran_seed", "c_unit", "dg_ext",
                 "dg_pool", "ad_annual", "conf_level", "digits"),
      time   = c("period_no", "year_start", "year_end", "period_type"),
      area   = c("trans_no", "trans_period", "lu_initial", "lu_final",
                 "trans_area", "trans_se", "trans_pdf", "trans_pdf_a",
                 "trans_pdf_b", "trans_pdf_c", "redd_activity"),
      carbon = c("c_no", "c_period", "c_element", "c_lu", "c_value",
  ...
  ## Emit a pass or fail message; mark all_ok FALSE on failure
   show_msg <- function(check, pass_label, fail_label, problems = NULL) {
    if (check) {
      message("\u2713 ", pass_label)
    } else {
  ...
  show_progress("Loading data...", step/n_steps*100)
  ...
  if (is.null(sheets_found)) {
    stop("Cannot read the file. Is it a valid .xlsx file?")
  }
  ...
  if (length(missing_tabsv1) == 0L) {
    ...
    stop("Missing required sheet(s): ", paste(missing_tabsv2, collapse = ", "))
  }
  ...
  show_progress("Checking column names...", step/n_steps*100)

  check_cols_setup  <- all(checks$cols$setup  %in% names(setup))
  check_cols_time   <- all(checks$cols$time   %in% names(time))
  check_cols_area   <- all(checks$cols$area   %in% names(area))
  check_cols_carbon <- all(checks$cols$carbon %in% names(carbon))
  ...
  show_progress("Checking table dimensions...", step/n_steps*100)
  ...
  show_progress("Checking column data types...", step/n_steps*100)
  ...
  check_dt_area <- all(
    ...
    is.character(area$lu_initial),
    is.character(area$lu_final),
  ...
  show_progress("Checking category values...", step/n_steps*100)
  ...
  check_cat_time <- all(stringr::str_detect(unique(time$period_type), pattern = ptype_pattern))
  ...
  check_cat_all <- all(check_cat_setup, check_cat_time, check_cat_area, check_cat_carbon)
  ...
  show_progress("Checking unique IDs...", step/n_steps*100)
  ...
  show_progress("Checking cross-table consistency...", step/n_steps*100)
  ...
  match_flags  <- c(check_match_period_area, check_match_period_carbon, check_match_lu,
  ...
  all_ok <- all(
    check_cols_all, check_size_all, check_dt_all, check_cat_all, check_ids_all,
    check_match_all
    )
