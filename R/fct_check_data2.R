...
fct_check_data2 <- function(.usr, .time, .ad, .cs){
  ...
  ## Revision v2.0
  app_checklist <- list(
    xlsx_tabs  = c("user", "time", "area", "carbon"),
    columns = list(
      usr = c(
        "trunc_pdf", "n_iter", "ran_seed", "c_unit", "dg_ext", "dg_pool", "ad_annual", "conf_level"),
      time = c("period_no", "year_start", "year_end", "period_type"),
      ad = c(
        "trans_id",	"trans_period",	"trans_placeholder",
      ...
      cs = c("c_no", "c_id", "c_period", "c_element", "c_lu_id", "c_placeholder",
                     "c_value", "c_se", "c_pdf", "c_pdf_a",	"c_pdf_b", "c_pdf_c", "c_lu_name")
    ),
    category = list(
      redd_activity = c("DF", "DG", "EN", "EN_AF", "EN_RE"),
      period_type   = c("REF", "REF[0-9]", "MON", "MON[0-9]"),
      c_units       = c("DM", "C"),
      c_pools       = c("AGB", "BGB", "DW", "LI", "SOC", "ALL"),
      c_elements    = c("CF", "RS", "DG_ratio", "C_all"),
      pdf           = c("normal", "beta")
    )
  )
  ...
  tmp$cols_usr_ok  <- all(app_checklist$columns$usr %in% names(.usr))
